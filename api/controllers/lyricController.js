...
export const generateLyric = async (req, res) => {
  try {
    ...
    const lyricSnippet = await geminiService.generateLyricSnippet(title, artist);
...
export const checkGuess = async (req, res) => {
  try {
    ...
    const { title, artist } = songService.extractTitleAndArtist(correctSong);
    
    // Use Gemini API for intelligent matching
    const result = await geminiService.checkGuessAccuracy(userGuess, title, artist);
